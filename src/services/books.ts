...
import { BookModel } from "../repositories/models/book";
...
export async function getAll(): Promise<Book[]> {
    const dtos: BookModel[] = await bookRepository.findAll();
...
export async function insert(request: CreateBookRequest): Promise<Book> {
    ...
    const dto: BookModel = new BookModel({ id: entity.id, authorId: entity.authorId, name: entity.name, publishDate: entity.publishDate, genre: entity.genre });
